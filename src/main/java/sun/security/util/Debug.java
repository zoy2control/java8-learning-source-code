...
public class Debug {
    ...
    static {
        // ·对 action授予特权。这个 action是 debug
        args = java.security.AccessController.doPrivileged
                (new sun.security.action.GetPropertyAction
                ("java.security.debug"));

        // ·对 action授予特权。这个 action是 auth.debug。authority
        String args2 = java.security.AccessController.doPrivileged
                (new sun.security.action.GetPropertyAction
                ("java.security.auth.debug"));

        if (args == null) {
            // ·CV·args赋值为auth.debug的 action
            args = args2;
        } else {
            // ·args和 args2都不为Null
            if (args2 != null) {
    ...
    /**
     * <p>
     *     ·入参option是否为正确的设置，是的话返回 新Debug对象
     * </p>
    ...
    /**
     * <p>
     *     ·若 system属性为"security.debug"，返回TRUE
     * </p>
    ...
    /**
     * <p>
     *     ·将 args：String每个字符转化为 小写，除了 class和 URL（正则表达式）
     * </p>
