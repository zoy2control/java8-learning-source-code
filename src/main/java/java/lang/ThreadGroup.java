...
class ThreadGroup implements Thread.UncaughtExceptionHandler {
    ...
    int nthreads; // ·线程数量
    Thread threads[];// ·数组存放线程

    int ngroups; // ·线程组数量
    ThreadGroup groups[]; // ·数组存放线程组
    ...
    /**
     * ·递归估算ThreadGroup以及其 子ThreadGroup的活跃线程数量
     * Returns an estimate of the number of active threads in this thread
     * group and its subgroups. Recursively iterates over all subgroups in
     * this thread group.
     *
     * ·之所以是 估计值，是因为 线程数量会动态的随着这个方法横穿数据结构内部而改变
    ...
    public int activeCount() {
        // ·准备数据
        int result; // ·记录 当前线程组以及 其子线程组的总线程数量
        // Snapshot sub-group data so we don't hold this lock
        // while our children are computing.
        int ngroupsSnapshot;
        ThreadGroup[] groupsSnapshot;
        // ·这里 this用的好，为下面 递归定了“锚”
        synchronized (this) {
            if (destroyed) {
                return 0;
            }
            result = nthreads; // ·当前线程组线程数量赋值为 result
            ngroupsSnapshot = ngroups;// ·当前线程组的 子线程组数量
            if (groups != null) {
                groupsSnapshot = Arrays.copyOf(groups, ngroupsSnapshot);
            } else {
                groupsSnapshot = null;
    ...
    /**
     * ·拷贝 线程组（可能包括所有子线程组，recurse标示符指定）的活跃线程到特定数组中
     * ·如果 特定数组不能装下那么多活跃的线程，那么这些 额外的线程将会被忽略
     *
     * Copies into the specified array every active thread in this
     ...
     *
     * ·上面的 .activeCount()方法可以估算出线程的数量，这个数量可以作为初始化数组大小的凭据
     *
     ...
     *
     * ·这个方法推荐用法是用于 debug或 monitor
     *
     * <p> Due to the inherent race condition in this method, it is recommended
     * that the method only be used for debugging and monitoring purposes.
     *
     * @param  list
     *         ·外部数组，横穿整个递归，用于存储活跃线程
     *         an array into which to put the list of threads
     *
     * @param  recurse
     *         ·是否需要递归计算 子线程组的线程数目
    ...
    private int enumerate(Thread list[], int n, boolean recurse) {
        ...
        synchronized (this) {
            ...
            // ·准备可拷贝进特定数组的 线程数目nt
            // ·在数组范围内：界限是 入参Thread[]的长度（入参Thread[]目标数组）
            int nt = nthreads;
            if (nt > list.length - n) {
                nt = list.length - n;
            }
            // ·找活跃线程：遍历当前线程组中活跃的线程，然后存放进 入参数组Threaad[]中
            for (int i = 0; i < nt; i++) {
    ...
    public int activeGroupCount() {
        ...
        // ·递归计算 子线程组数目
        for (int i = 0 ; i < ngroupsSnapshot ; i++) {
            n += groupsSnapshot[i].activeGroupCount(); // ·当前 + 子
        }
        return n;
    }
    ...
    private int enumerate(ThreadGroup list[], int n, boolean recurse) {// 入参n是 入参线程组ThreadGroup[]的线程组数量
        int ngroupsSnapshot = 0;
        ThreadGroup[] groupsSnapshot = null;
        synchronized (this) {
            if (destroyed) {
            ...
            // ·数组限定：ng：可存入ThreadGroup[]的线程组数量
            int ng = ngroups;
            if (ng > list.length - n) {
                ng = list.length - n;
            }
            // ·持久化 入参ThreadGroup[]
            // ·？？？区别 for():[] = []。==》其实两种方法都可以的，只是实现不一样
            // ·？？？哪种效率高
            if (ng > 0) {
                // ·将 当前线程组groups拷贝到 入参ThreadGroup[] list中（即 统一存放的list）
                System.arraycopy(groups, 0, list, n, ng);
                n += ng; // ·更新 入参ThreadGroup[]数组的元素数量
            }
            // ·如果需要 遍历子线程组，不用 CV·groups而用 V·groupsSnapshot
            // ·需要准备 当前线程组备份groupsSnapshot，便于递归不改变原子线程组
            if (recurse) {
                ngroupsSnapshot = ngroups;
                if (groups != null) {
                    groupsSnapshot = Arrays.copyOf(groups, ngroupsSnapshot);
    ...
    public final void destroy() {
        ...
        synchronized (this) {
            ...
            // ·备份 线程组groupsSnapshot
            // ·==》 备份的原因是，在 锁内设置 线程组被销毁之后（如果没备份就不可以调用方法了），还可以在 锁外执行 .destroy()等线程方法
            ngroupsSnapshot = ngroups;
            if (groups != null) {
                groupsSnapshot = Arrays.copyOf(groups, ngroupsSnapshot);
            } else {
    ...
    void list(PrintStream out, int indent) {
        ...
        synchronized (this) {
            ...
            // ·备份groupsSnapshot：ThreadGroup[]（锁内更新V）
            ngroupsSnapshot = ngroups;
            if (groups != null) {
                groupsSnapshot = Arrays.copyOf(groups, ngroupsSnapshot);
            } else {
        ...
        // ·遍历线程组，并递归调用 .list()打印线程组信息（锁外使用V）
        for (int i = 0 ; i < ngroupsSnapshot ; i++) {
